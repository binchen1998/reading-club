...
def _series_cover(series_id: str) -> str:
    row = next((item for item in (json.loads(CATALOG.read_text(encoding="utf-8")).get("series") or []) if item.get("id") == series_id), None)
    books = (row or {}).get("books") or []
    if not books:
        return ""
    first = books[0]
    slug = book_slug_of(first.get("title") or "", first.get("name") or "")
    return page_image_url(series_id, slug, 1)
